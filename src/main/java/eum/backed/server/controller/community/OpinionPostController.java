...
public class OpinionPostController {
    ...
    @PostMapping()
    public ResponseEntity<APIResponse> create(@RequestBody OpinionPostRequestDTO.Create create , @AuthenticationPrincipal String email){
        return ResponseEntity.ok(opinionPostService.create(create, email));
    }
    @ApiOperation(value = "의견 게시글 수정", notes = "의견 게시글 수정")
    @PutMapping("/{postId}")
    public  ResponseEntity<APIResponse> update(@PathVariable Long postId,@RequestBody OpinionPostRequestDTO.Update update,@AuthenticationPrincipal String email){
...
//    }
    ...
    public  ResponseEntity<APIResponse<List<OpinionResponseDTO.AllOpinionPostsResponses>>>findByFilter(@RequestParam(name = "search",required = false) String keyword,@RequestParam(name = "hottest",required = false ) String isShow, @AuthenticationPrincipal String email){
        return ResponseEntity.ok(opinionPostService.findByFilter(keyword,isShow,email));
    }

    @GetMapping("/{postId}")
    @ApiOperation(value = "단일 의견 게시글 조회",notes = "입력받은 게시글의 정보 + 댓글")
    public  ResponseEntity<APIResponse<OpinionResponseDTO.OpinionPostWithComment>> getOpinionPostWithComment(@PathVariable Long postId){
        return ResponseEntity.ok(opinionPostService.getOpininonPostWithComment(postId));
    }
